...
**做了什么**：
- 在 `.cline/rules/project_context.mdc` 中新增 **Agent 工作流规范** 独立章节
- 4 步强制流程：更新日志 → Git 提交 → 报告+列下一步（询问式） → 检查待办池新增项
- 从 Git 工作流中移除重复的"日志同步"规则，改为由 Agent 工作流统一覆盖
- 更新远程仓库信息（从"仅本地"改为已推送 4 commit 到 `origin/master`）

**产出物**：
- `project_context.mdc` 新增章节，Agent 行为从此有硬性规则约束

**决策**：
- 采用**询问式**（模式 A）：每次完成报告后列出下一步计划，等待用户确认，不自动推进
- 新增第 4 步"检查待办池新增项"，确保伴随子任务不被遗漏
...
- [ ] 阶段 10: 数据导入 — 从 kg_system 白名单迁移下腰痛知识图谱节点和边数据
- [ ] 阶段 11: 端到端集成测试（实体抽取 → 推理 → 生成）
- [ ] 阶段 12: 演示 Notebook
- [ ] 新增: 单元测试覆盖 reasoning/rule_engine（规则加载/匹配）
- [ ] 新增: 单元测试覆盖 reasoning/confidence（评分聚合）
...
> - 每次任务完成后必须执行 Agent 工作流规范（更新日志 → Git 提交 → 报告+列下一步）
